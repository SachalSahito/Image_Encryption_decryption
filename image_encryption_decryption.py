...
def open_img():
    global panelA, panelB,x
    x = openfilename()
    if panelA is None or panelB is None:
        panelA = Label(window)
        panelA.place(x=50, y=250)
        panelB = Label(window)
        panelB.place(x=650, y=250)
    display_image(x, panelA)
    display_image(x, panelB)
...
def save_img():
    ...
    eimg = Image.open('image_encrypted.jpg')
...
start1 = tk.Label(text="Original\nImage", font=("Arial", 30), fg="magenta")  # same way bg
start1.place(x=100, y=200)

# edited image label
start1 = tk.Label(text="Encrypted\nDecrypted\nImage", font=("Arial", 30), fg="magenta")  # same way bg
start1.place(x=700, y=200)
